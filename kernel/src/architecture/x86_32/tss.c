...
int init_tss() {
    serial_puts("init_tss() Setting up TSS entries...");
    set_tss_entry(5, 0x10, 0x0100000);
    serial_puts(" OK\n");

    serial_puts("init_tss() Loading the TSS into the CPU...");
    load_tss();
    serial_puts(" OK (Warning : if the TSS is invalid, the IDT will call the Invalid TSS exception handler)\n");
