...
void init_idt() {
    serial_puts("init_idt() Setting up IDT pointer...");
    g_idt_ptr.base_address = (uint32_t)g_idt;
    g_idt_ptr.limit = sizeof(g_idt) - 1;
    serial_puts(" OK\n");

    serial_puts("init_idt() Initializing PIC...");
    pic8259_init();
    serial_puts(" OK\n");

    serial_puts("init_idt() Setting up IDT entries...");
    ...
    serial_puts("init_idt() Loading the IDT into the CPU...");
    load_idt((uint32_t)&g_idt_ptr);
    serial_puts(" OK\n");

    serial_puts("init_idt() Enabling interrupts...");
    asm volatile("sti");
    serial_puts(" OK\n");
